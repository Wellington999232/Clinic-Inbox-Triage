...
def test_appointment_request_no_red_flag():
    text = "can I move my appointment to Thursday please"
    matched, tags = check_red_flags(text)
    assert matched is False

def test_routine_swelling_no_red_flag():
    text = "I have some swelling after my lip filler is that normal"
    matched, tags = check_red_flags(text)
    assert matched is False

def test_red_flag_match_forces_escalation():
    label, severity, override = apply_policy_overrides(
        label=Label.routine_clinical_question,
...
def test_low_confidence_routes_to_urgent():
    label, severity, override = apply_policy_overrides(
        label=Label.post_treatment_reassurance,
        severity=Severity.low,
        confidence=0.3,
...
def test_high_confidence_clean_message_no_override():
    label, severity, override = apply_policy_overrides(
        label=Label.scheduling_admin,
        severity=Severity.low,
        confidence=0.95,
        red_flag_matched=False,
    )
