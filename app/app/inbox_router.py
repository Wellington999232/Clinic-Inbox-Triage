...
def submit_message(submission: PatientSubmission, db: Session = Depends(get_db)):
    """
    Patient submits a message. It gets classified immediately
    and stored in the database with the triage result.
    """
    msg_id = str(uuid.uuid4())

    triage = classify_message(MessageInput(
        id=msg_id,
        text=submission.message_text
    ))
...
def get_messages(db: Session = Depends(get_db)):
    """
    Returns all messages sorted by severity — high first.
    This is the clinician inbox view.
    """
...
def send_reply(request: ReplyRequest, db: Session = Depends(get_db)):
    """
    Clinician sends a reply to a patient message.
    Marks the message as replied and logs the action.
    """
    message = db.query(PatientMessage).filter(
