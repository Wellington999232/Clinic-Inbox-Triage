...
def submit_message(submission: PatientSubmission, db: Session = Depends(get_db)):
    ...
    audit = AuditLog(
        ...
        action="classified",
        performed_by="system",
        detail="Classified as " + triage.primary_label.value,
...
def send_reply(request: ReplyRequest, db: Session = Depends(get_db)):
    ...
    audit = AuditLog(id=uuid.uuid4(), message_id=request.message_id, action="replied", performed_by=request.replied_by, detail="Reply sent", timestamp=datetime.utcnow())
    db.add(audit)
    db.commit()
    return {"message_id": request.message_id, "status": "replied", "replied_by": request.replied_by, "replied_at": message.replied_at.isoformat()}
